...
const SearchPorOrgano = async () => {
    try {
        return await Cassette.findAll(
            {
                order: [['organo', 'ASC']], // Ordena por fecha ascendente
                limit: 20
            }
        );
    } catch (error) {
...
// BUSCAR CASSETTES POR DESCRIPCION
const SearchPorDescripcion = async () => {
    try {
        return await Cassette.findAll(
            {
                order: [['fecha', 'DESC']], // Ordena por fecha ascendente
                limit: 20
            }
        );
    } catch (error) {
        console.error("Error buscando cassettes por descripción:", error);
        throw error;
    }
};

// BUSCAR CASSETTES POR FECHA
const SearchPorFecha = async () => {
    try {
        return await Cassette.findAll(
            {
                order: [['descripcion', 'DESC']], // Ordena por fecha ascendente
                limit: 20
            }
        );
    } catch (error) {
