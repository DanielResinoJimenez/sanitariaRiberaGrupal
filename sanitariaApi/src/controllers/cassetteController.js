...
const getPorOrgano = async (req, res) => {
    try {
        const cassette = await cassetteService.SearchPorOrgano();
        cassette
            ? res.status(200).json(cassette)
...
// OBTENER CASSETTE POR ORGANO
const getPorDescripcion = async (req, res) => {
    try {
        const cassette = await cassetteService.SearchPorDescripcion();
        cassette
            ? res.status(200).json(cassette)
            : res.status(404).json({ message: "No se ha encontrado ningún cassette para esa descripción" });
    } catch (error) {
        res.status(500).json({ error: error.message });
    }
};
...
const getPorFecha = async (req, res) => {
    try {
        const cassette = await cassetteService.SearchPorFecha();
        cassette
            ? res.status(200).json(cassette)
